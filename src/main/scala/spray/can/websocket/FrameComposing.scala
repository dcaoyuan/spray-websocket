  ...
  def apply(maxMessageLength: Long) = new PipelineStage {
    def apply(context: PipelineContext, commandPL: CPL, eventPL: EPL): Pipelines = new Pipelines {
      ...
      val eventPipeline: EPL = {
        ...
        case FrameInEvent(x @ Frame(false, _, Opcode.Text | Opcode.Binary, _)) if infinFrame.isEmpty =>
          infinFrame = Some(x)

        case FrameInEvent(ContinuationFrame(false, _, payload)) if infinFrame.isDefined =>
        ...
        case FrameInEvent(x @ Frame(true, _, Opcode.Continuation | Opcode.Text | Opcode.Binary, payload)) =>
          if (x.opcode == Opcode.Continuation && infinFrame.isEmpty || x.opcode != Opcode.Continuation && infinFrame.isDefined) {
            commandPL(Tcp.Close)
