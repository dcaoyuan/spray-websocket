      ...
      val eventPipeline: EPL = {
        ...
        case FrameInEvent(x @ Frame(false, _, Opcode.Text | Opcode.Binary, _)) =>
          infinFrame match {
            ...
            case _ => closeWithReason(StatusCode.ProtocolError,
        ...
        case FrameInEvent(ContinuationFrame(false, _, payload)) =>
          infinFrame match {
            ...
            case None => closeWithReason(StatusCode.ProtocolError,
        ...
        case FrameInEvent(x @ Frame(true, _, Opcode.Continuation | Opcode.Text | Opcode.Binary, payload)) =>
          ...
          } else if (infinFrame.map(_.payload.length).getOrElse(0) + x.payload.length > messageSizeLimit) {
