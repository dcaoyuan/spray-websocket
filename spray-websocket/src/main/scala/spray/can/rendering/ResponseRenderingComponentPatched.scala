...
private[can] trait ResponseRenderingComponentPatched {
  ...
  def renderResponsePartRenderingContext(r: Rendering, ctx: ResponsePartRenderingContext,
        ...
        remaining match {
          case head :: tail ⇒ head match {
            ...
            case x: `Connection` ⇒
              ...
              if (has(x.tokens, "upgrade")) render(x) // Patched, @see https://github.com/spray/spray/pull/836
    ...
    // Patched, @see https://github.com/spray/spray/pull/836
    @tailrec def has(tokens: Seq[String], item: String, ix: Int = 0): Boolean =
      if (ix < tokens.length)
        if (tokens(ix) equalsIgnoreCase item) true
        else has(tokens, item, ix + 1)
      else false
